...
export async function POST(request: NextRequest) {
  // Guard: instantiate inside handler so a missing key doesn't crash the build
  const secretKey = process.env.LIVEBLOCKS_SECRET_KEY;
  if (!secretKey || !secretKey.startsWith("sk_")) {
    console.error(
      "❌ LIVEBLOCKS_SECRET_KEY is missing or invalid. Get it from https://liveblocks.io/dashboard/apikeys",
    );
    return new NextResponse("Liveblocks secret key not configured", {
      status: 500,
    });
  }
